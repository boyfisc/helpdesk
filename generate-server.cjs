...
app.get('/api/tickets/public', async (req: Request, res: Response) => {
  ...
  const { data, error } = await supabaseAdmin
    .from('tickets')
    .select('id, ticket_number, object_type, platform, centre_fiscal, created_at, status')
...
app.get('/api/tickets/private', async (req: Request, res: Response) => {
  if (!supabaseAdmin) return res.status(500).json({ error: 'Supabase non configuré' });
...
async function logAudit(action: string, details: string, email: string = 'system@dgid.sn', name: string = 'Système', role: string = 'SYSTEM') {
